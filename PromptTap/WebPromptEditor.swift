...
extension WebPromptEditor {
    fileprivate var webShortcuts: [String: WebShortcutDescriptor] {
        [
    ...
    fileprivate func shortcutDescriptor(for action: KeyboardShortcutAction) -> WebShortcutDescriptor {
        (shortcuts[action] ?? action.defaultHotkey).webShortcutDescriptor
    }

    fileprivate static let editorHTML = """
        <!doctype html>
        <html>
        <head>
          <meta charset="utf-8">
